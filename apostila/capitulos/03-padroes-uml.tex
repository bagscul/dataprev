...
\begin{jacaiu}
\textbf{Padrões (23 questões, subtag \texttt{padroes-projeto}):} os cinco
criacionais em cenário (Singleton para instância única de configuração e de
\emph{logging}; Abstract Factory para famílias de componentes; Builder com
muitos atributos opcionais; Prototype quando criar do zero é caro; Factory
...
a \textbf{classificação dos 23} em três famílias; MVC; e GRASP (a qual classe
cabe a responsabilidade). Rode \texttt{./quiz.py padroes-projeto}.
\end{jacaiu}
...
\begin{jacaiu}
\textbf{UML (10 questões, subtag \texttt{uml}):} as duas famílias na UML 2.x;
composição $\times$ agregação; multiplicidade em diagrama de classes;
\texttt{<<include>>} $\times$ \texttt{<<extend>>}; sequência para a ordem
temporal das mensagens; atividade para decisão, paralelismo e sincronização;
máquina de estados no ciclo de vida do objeto; generalização (direção da
seta); componentes $\times$ implantação; realização de interface. Rode
\texttt{./quiz.py uml}.
\end{jacaiu}
