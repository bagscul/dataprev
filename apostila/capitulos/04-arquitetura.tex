...
\begin{comosair}
Vertical = SCALE UP (uma máquina maior); horizontal = SCALE OUT (mais
máquinas). REST: stateless, cache no cliente, interface uniforme, escala
porque o servidor não guarda sessão. SOAP: contrato WSDL/XML, mais pesado,
estilo RPC. Gatilhos de distrator neste bloco: ``sempre'', ``exclusivamente'',
``elimina a necessidade de'', ``idêntico ao''.
\end{comosair}
