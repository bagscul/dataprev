...
\item \textbf{Blockchain:} ver Capítulo \ref{cap:programacao}.
\end{itemize}
...
Memorize os pares: AD $\times$ DBA, incremental $\times$ diferencial, RPO
$\times$ RTO, ordem do ARIES, bitmap $\times$ B+ Tree, rotulado $\times$ não
rotulado, data $\times$ concept drift, L1 $\times$ L2, Tipo 1 $\times$ Tipo
2, over $\times$ underfitting. Rode \texttt{./quiz.py orfaos} --- mistura DBA,
administração de dados, processos e temas avulsos que caíram nas provas
reais.
\end{jacaiu}
