...
reais no corpus (68), e a maioria esmagadora é \textbf{DBA}. \textbf{ARIES e
...
engenharia de software (RUP, cascata, casos de uso, ponto de função) e UML
(sequência, composição, diagrama de objetos).

\textbf{Arquitetura de computadores e SO} entrou aqui no fechamento do ITEM
7: 11 questões que vinham marcadas como \texttt{arquitetura} --- gargalo de
\textbf{Von Neumann}, ULA e unidade de controle, \emph{overflow} $\times$
\emph{carry}, complemento de dois, conversão de base, ROM/firmware,
periférico, ciclo busca-decodificação-execução, \textbf{MMU/TLB} e
\emph{thrashing}, tabela de páginas, \textbf{DMA}. São de provas de
\textbf{outro perfil} (ALERO 2026 e TJ-RJ) e \textbf{estão fora do edital do
Perfil 3}, que fala em arquitetura \emph{de software} (Capítulo
\ref{cap:arquitetura}). Ficam catalogadas, não estudadas.
