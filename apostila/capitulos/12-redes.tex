...
\begin{jacaiu}
Camada OSI de sessão (checkpoints); switch $\times$ roteador $\times$ hub;
TCP $\times$ UDP; portas; RFC 1918; IPv6 128 bits; roteamento estático
$\times$ OSPF (distância administrativa); ACL Cisco (wildcard mask). Rode
\texttt{./quiz.py redes}.
\end{jacaiu}
