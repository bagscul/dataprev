...
\textbf{análise} para decisões \textbf{não estruturadas ou
semiestruturadas} --- simulações, cenários “e se”. Atende tipicamente a
gerência \textbf{média e sênior}, não necessariamente o topo estratégico
puro (esse é o nicho do SIE/EIS --- Sistema de Informação Executiva ---,
menos cobrado, focado em indicadores de altíssimo nível para a alta
direção).
...
estratégico no topo) ajuda a lembrar a ordem crescente de abstração: SPT
nasce na base, SIG no meio, SAD acima do SIG (mas não necessariamente no
topo absoluto); ERP/CRM/SCM são \textbf{transversais}, cortando vários
níveis ao mesmo tempo, cada um por um recorte de processo diferente
(recursos internos, cliente, cadeia de fornecimento).
\end{conceito}
